...
# Important note:
# If you don't synchronize but then edit the other file, the newer modification
# time on the second file edited will cause data to be overridden.
#
# The solution would be to merge manually (Database --> merge from database).
...
function sync_passwords()
{
    ...
    # The conversion is required for the comparison in the following if stmt
    ...
    # Handle remote being newer than local
        ...
        printf "Passwords file is already synchronized.\n"
        return 0
    fi
}
